...
interface Props { cfg: SessionConfig; setCfg: Dispatch<SetStateAction<SessionConfig>>; devices: AudioDevice[]; levels: Levels | null }
        ...
        return (
          <div key={s.device_id} className={`strip ${s.muted ? 'muted' : ''}`}>
            <div className="strip-head">
              <span className="strip-name" title={name}>{s.role === 'local' ? 'You' : 'Call'} · {name}</span>
              <button className="mute" aria-pressed={s.muted} onClick={() => update(s.device_id, { muted: !s.muted })}>{s.muted ? 'Unmute' : 'Mute'}</button>
            </div>
            <div className="meter" aria-hidden><span style={{ width: `${Math.max(0, (db + 60) / 60) * 100}%` }} /></div>
            <input type="range" min={0} max={2} step={0.05} value={s.gain} aria-label={`${name} gain`} onChange={e => update(s.device_id, { gain: Number(e.target.value) })} />
          </div>
        )
      })}
    </section>
