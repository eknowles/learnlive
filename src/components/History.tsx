...
export default function History() {
  ...
  useEffect(() => { api.listMeetings().then(setMeetings); api.getRememberVoices().then(setRemember) }, [])
  useEffect(() => {
    if (!q.trim()) { setHits([]); return }
    const t = setTimeout(() => api.search(q).then(setHits).catch(() => setHits([])), 200)
  ...
  const remove = async (id: number) => {
    ...
    await api.deleteMeeting(id); setOpen(null); setMeetings(await api.listMeetings())
  }
  const toggleRemember = async (on: boolean) => {
    ...
    await api.setRememberVoices(on); setRemember(on)
  }
    ...
    return (
      <div className="history">
        <div className="history-head">
          ...
          <span className="quiet">{day(m.started_at)} · {clock(m.started_at * 1000).slice(0, 5)} · {mins(m)} · {segments.length} sentences</span>
  ...
  return (
    <div className="history">
      <div className="history-head">
        ...
        <input className="search" placeholder="Search everything that was said…  (e.g. книгу, book, instrument*)" value={q} onChange={e => setQ(e.target.value)} />
      </div>
      ...
      {q.trim() ? (
        <ul className="hits">
          ...
          {hits.map(h => (
            <li key={h.segment.id}>
              <button className="linklike" onClick={() => openMeeting(h.meeting_id)}>{h.meeting_title}</button>
              <span className="quiet"> · {day(h.started_at)} · {h.segment.speaker.label}</span>
        ...
        <ul className="meetings">
          ...
          {meetings.map(m => (
            <li key={m.id}>
              <button className="linklike title" onClick={() => openMeeting(m.id)}>{m.title}</button>
              <span className="quiet">{day(m.started_at)} · {clock(m.started_at * 1000).slice(0, 5)} · {mins(m)} · {m.sentence_count} sentences{m.participants.length ? ` · ${m.participants.map(p => p.name).join(', ')}` : ''}</span>
            </li>
          ))}
        </ul>
      )}
