...
export default function Setup({ cfg, setCfg, devices, languages, models, progress, phase, onStart, onStop }: Props) {
  ...
  const setSource = (role: 'remote' | 'local', device_id: string) =>
    setCfg(c => ({ ...c, sources: [
      ...c.sources.filter(s => s.role !== role),
      ...(device_id ? [{ device_id, role, gain: 1, muted: false }] : []),
    ] }))
    ...
    <section className="setup" aria-label="Session setup">
      ...
      <label>
        ...
        <select value={remote?.device_id ?? ''} disabled={phase === 'live'} onChange={e => setSource('remote', e.target.value)}>
          ...
          {devices.map(d => <option key={d.id} value={d.id}>{d.name}{d.is_loopback ? '' : ' (not a loopback device)'}</option>)}
        </select>
        {loopback.length === 0 && <small className="hint">No loopback device found. Install BlackHole and set it as your call's speaker.</small>}
      </label>
      <label>
        Your microphone
        <select value={local?.device_id ?? ''} disabled={phase === 'live'} onChange={e => setSource('local', e.target.value)}>
          <option value="">None</option>
          {devices.filter(d => !d.is_loopback).map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
        </select>
      </label>
      ...
      <details>
        ...
        <label>
          Recognition quality
          <select value={cfg.asr_model} disabled={phase === 'live'} onChange={e => set('asr_model', e.target.value as SessionConfig['asr_model'])}>
        ...
        <label className="row">
          <input type="checkbox" checked={cfg.speak_translations} disabled={phase === 'live'} onChange={e => set('speak_translations', e.target.checked)} />
          Read translations aloud
        </label>
        {cfg.speak_translations && (
          <label>
            Lower the call while reading · {Math.round(cfg.duck_amount * 100)}%
            <input type="range" min={0} max={1} step={0.05} value={cfg.duck_amount} onChange={e => set('duck_amount', Number(e.target.value))} />
          </label>
        )}
      </details>

      {phase === 'live'
        ? <button className="primary stop" onClick={onStop}>Stop listening</button>
        : <button className="primary" onClick={onStart} disabled={busy || cfg.sources.length === 0}>
            {phase === 'downloading' ? 'Downloading models…' : phase === 'starting' ? 'Loading models…' : missing.length ? `Download ${missingMb} MB and start` : 'Start listening'}
          </button>}
      ...
      {phase === 'downloading' && (
        <ul className="downloads">
          {missing.map(m => {
            ...
            return <li key={m.id}><span>{m.id}</span><progress value={pct} max={100} /><span>{p?.done ? 'done' : p ? `${mb(p.bytes)} MB` : 'queued'}</span></li>
          })}
        </ul>
      )}
    </section>
