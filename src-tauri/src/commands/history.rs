...
pub fn assign_speaker(state: State<AppState>, meeting_id: i64, speaker_id: u32, name: String, email: Option<String>) -> CmdResult<MeetingSummary> {
    ...
    if name.is_empty() { return Err(AppError::invalid("Name can't be empty")); }
    ...
    Ok(state.db.meeting_summary(meeting_id)?.ok_or_else(|| AppError::invalid("meeting not found"))?)
}
...
pub fn delete_meeting(state: State<AppState>, id: i64) -> CmdResult<()> {
    for clip in state.db.delete_meeting(id)? { let _ = std::fs::remove_file(clip); }
    Ok(())
}
...
#[tauri::command]
pub fn set_remember_voices(state: State<AppState>, on: bool) -> CmdResult<()> { Ok(state.db.set_remember_voices(on)?) }

#[tauri::command]
pub fn forget_voice(state: State<AppState>, participant_id: i64) -> CmdResult<()> { Ok(state.db.forget_voice(participant_id)?) }
