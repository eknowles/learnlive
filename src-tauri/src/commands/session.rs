...
pub fn list_languages() -> Vec<Language> {
    LANGUAGES.iter().map(|l| Language { tts: crate::models::tts(l.code).is_some(), ..l.clone() }).collect()
}
...
pub async fn start_session(app: AppHandle, state: State<'_, AppState>, cfg: SessionConfig, event: Option<CalendarEvent>) -> CmdResult<i64> {
    if cfg.sources.is_empty() { return Err(AppError::invalid("Pick at least one audio source")); }
    if crate::engine::languages::by_code(&cfg.learning).is_none() || crate::engine::languages::by_code(&cfg.native).is_none() {
    ...
    if let Some(e) = &event { state.db.link_event(meeting_id, &e.id, &e.title, &attendees(e))?; }

    let (engines, db, app2) = (state.engines.clone(), state.db.clone(), app.clone());
    let handle = tauri::async_runtime::spawn_blocking(move || pipeline::start(app2, engines, db, meeting_id, cfg, clip_dir)).await??;
...
pub fn update_mixer(state: State<AppState>, source: MixerSource) -> CmdResult<()> {
    ...
    let m = s.mixer();
    m.send(crate::audio::MixerCommand::SetGain { device_id: source.device_id.clone(), gain: source.gain }).map_err(|e| AppError::audio(e.to_string()))?;
    m.send(crate::audio::MixerCommand::SetMuted { device_id: source.device_id, muted: source.muted }).map_err(|e| AppError::audio(e.to_string()))?;
    Ok(())
}
...
fn finish(state: &State<AppState>, s: pipeline::SessionHandle) {
    ...
    if !state.db.remember_voices() { return; }
    let assigned: std::collections::HashMap<u32, i64> = state.db.meeting_summary(s.meeting_id).ok().flatten()
    ...
    for (sid, known_pid, centroid, n) in s.speakers.lock().centroids() {
        if sid == 0 || n == 0 { continue; }
        if let Some(pid) = assigned.get(&sid).copied().or(known_pid) { let _ = state.db.update_voiceprint(pid, &centroid, n); }
    }
}
