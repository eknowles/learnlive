...
#[derive(Deserialize)]
struct Line { speaker: String, voice: String, #[serde(default = "remote")] role: String, text: String, #[serde(default = "gap")] pause_ms: u64 }
fn remote() -> String { "remote".into() }
fn gap() -> u64 { 900 }
...
fn main() -> Result<()> {
    ...
    let rest: Vec<&String> = args.iter().enumerate().filter(|(i, _)| *i != 0 && *i != mi && *i != mi + 1).map(|(_, a)| a).collect();
    let (script, out) = (rest.first().ok_or_else(|| anyhow!("script path"))?, rest.get(1).ok_or_else(|| anyhow!("out stem"))?);
    ...
    lanes[0].resize(8000, 0.0); lanes[1].resize(8000, 0.0);

    for l in &lines {
        if !voices.contains_key(&l.voice) {
            let v = Piper::load(&models, &l.voice)?.with_context(|| format!("no Piper voice for '{}' in {}", l.voice, models.display()))?;
        ...
        for x in lanes.iter_mut() { x.resize(start, 0.0); }
        lanes[lane].extend_from_slice(&pcm16);
        lanes[1 - lane].resize(lanes[lane].len(), 0.0);
        let dur_ms = pcm16.len() as u64 * 1000 / 16_000;
        expected.push(Expected { speaker: l.speaker.clone(), role: l.role.clone(), lang: l.voice.clone(), text: l.text.clone(), start_ms: clock_ms, end_ms: clock_ms + dur_ms });
        clock_ms += dur_ms + l.pause_ms;
        let pad = (l.pause_ms * 16) as usize;
        for x in lanes.iter_mut() { let n = x.len(); x.resize(n + pad, 0.0); }
    }

    let n = lanes[0].len().max(lanes[1].len());
    for x in lanes.iter_mut() { x.resize(n, 0.0); }
