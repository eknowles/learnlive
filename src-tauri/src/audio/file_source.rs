...
pub fn read_wav(path: &Path) -> Result<Wav> {
    ...
    if b.len() < 12 || &b[0..4] != b"RIFF" || &b[8..12] != b"WAVE" { return Err(anyhow!("not a RIFF/WAVE file")); }
    ...
    let samples: Vec<f32> = match (tag, bits) {
        ...
        (1, 24) => data.chunks_exact(3).map(|c| (i32::from_le_bytes([0, c[0], c[1], c[2]]) >> 8) as f32 / 8_388_608.0).collect(),
        (1, 32) => data.chunks_exact(4).map(|c| i32::from_le_bytes(c.try_into().unwrap()) as f32 / 2_147_483_648.0).collect(),
...
pub fn write_wav16(path: &Path, rate: u32, channels: u16, interleaved: &[f32]) -> Result<()> {
    let data: Vec<u8> = interleaved.iter().flat_map(|s| ((s.clamp(-1.0, 1.0) * 32767.0) as i16).to_le_bytes()).collect();
    ...
    buf.extend(16u32.to_le_bytes()); buf.extend(1u16.to_le_bytes()); buf.extend(channels.to_le_bytes());
    buf.extend(rate.to_le_bytes()); buf.extend((rate * block).to_le_bytes()); buf.extend((block as u16).to_le_bytes()); buf.extend(16u16.to_le_bytes());
...
pub fn feed(device_id: String, wav: Wav, tx: Sender<RawChunk>, realtime: bool) -> Result<()> {
    ...
    for chunk in wav.samples.chunks(step) {
        tx.send(RawChunk { device_id: device_id.clone(), sample_rate: wav.sample_rate, channels: wav.channels, samples: chunk.to_vec() })
            .map_err(|_| anyhow!("mixer went away"))?;
        if realtime { thread::sleep(Duration::from_millis(20)); }
    }
    Ok(())
}
